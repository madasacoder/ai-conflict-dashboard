...
const DOMPURIFY_CONFIG = {
  ...
  ALLOWED_ATTR: ['href', 'title', 'target', 'rel', 'class', 'id', 'src', 'alt', 'width', 'height'],
...
function setInnerHTMLSafely(element, html, config = {}) {
  ...
  // Safe because safeHTML is already sanitized by DOMPurify
...
function createElementSafely(html) {
  ...
  // Safe because we're using sanitizeHTML
  // eslint-disable-next-line no-unsanitized/property
...
function setupXSSMonitoring() {
  ...
  Object.defineProperty(Element.prototype, 'innerHTML', {
    set: function (value) {
      ...
      const isSafeCall =
        stack.includes('setInnerHTMLSafely') || stack.includes('processTextWithHighlightingSafe');
