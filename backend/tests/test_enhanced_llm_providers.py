...
class TestEnhancedOpenAIIntegration:
    ...
    async def test_openai_response_quality_validation(self):
        ...
        with patch("llm_providers._call_openai_with_breaker", new_callable=AsyncMock) as mock_call:
            ...
            assert not result["response"].startswith(' '), "Response shouldn't start with whitespace"
            assert not result["response"].endswith('...'), "Response shouldn't trail off"
            
            # Token economics validation
            assert result["tokens"]["completion"] > result["tokens"]["prompt"], "Should generate more than input"
        ...
        with patch("llm_providers._call_openai_with_breaker", new_callable=AsyncMock) as mock_call:
            ...
            with patch("llm_providers.RETRY_DELAYS", [0.1, 0.2, 0.4]):
                ...
                if len(attempt_times) > 2:
                    ...
                    assert 0.15 < second_gap < 0.25, f"Second retry gap {second_gap} not in expected range"
...
class TestEnhancedMultiModelComparison:
    ...
    async def test_model_response_consensus_detection(self):
        ...
        assert all(abs(l - avg_length) / avg_length < 0.3 for l in lengths), \
    ...
    async def test_model_response_conflict_detection(self):
        ...
        has_buy = any(
            keyword in responses[0]["response"].lower() 
            for keyword in buy_keywords
        )
        has_sell = any(
            keyword in responses[1]["response"].lower() 
...
class TestEnhancedErrorHandling:
    ...
    async def test_graceful_degradation_cascade(self):
        ...
        with patch("llm_providers._call_openai_with_breaker", new_callable=AsyncMock) as mock_openai:
            with patch("llm_providers._call_claude_with_breaker", new_callable=AsyncMock) as mock_claude:
                with patch("llm_providers._call_gemini_with_breaker", new_callable=AsyncMock) as mock_gemini:
                    ...
                    results = await analyze_with_models(
                        ...
                        {"openai_model": "gpt-4", "claude_model": "claude-3", "gemini_model": "gemini-pro"}
                    )
...
class TestEnhancedSecurity:
    ...
    async def test_api_key_sanitization_in_all_paths(self):
        ...
        with patch("llm_providers._call_openai_with_breaker", new_callable=AsyncMock) as mock_call:
            ...
            assert "sk-1234..." in result.get("error", "") or \
                   "API key" in result.get("error", ""), \
