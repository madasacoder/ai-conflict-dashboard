...
class TestRealBackendIntegration:
    ...
    def test_concurrent_real_requests_find_race_conditions(self):
        ...
        def make_request(i):
            ...
            response = requests.post(
                f"{BASE_URL}/api/analyze",
                json={
                    "text": f"Concurrent request {i}: Analyze this.",
                    "openai_key": f"key-{i}"
                },
                timeout=10
            )
    ...
    def test_real_cors_headers_for_browser_compatibility(self):
        ...
        assert "content-type" in cors_headers.get("access-control-allow-headers", "").lower(), \
...
class TestRealOllamaIntegration:
    ...
    def test_real_ollama_generation(self):
        ...
        assert "test" in data["response"].lower() or "success" in data["response"].lower(), \
    ...
    def test_backend_ollama_integration(self):
        ...
        if response.status_code == 200:
            ...
            if ollama_responses:
                ...
                assert ollama_resp.get("response") or ollama_resp.get("error"), \
...
class TestRealEndToEnd:
    ...
    def test_complete_analysis_workflow(self):
        ...
        assert len(models_responded) > 0 or len(models_failed) > 0, \
    ...
    def test_rapid_sequential_requests_reveal_state_issues(self):
        ...
        assert len(unique_ids) == len(request_ids), \
