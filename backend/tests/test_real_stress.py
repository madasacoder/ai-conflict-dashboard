    ...
    def test_request_size_limits(self):
        ...
        for size_kb in sizes_kb:
            ...
            try:
                response = requests.post(
                    f"{BASE_URL}/api/analyze",
                    json={"text": text, "openai_key": "test"},
                    timeout=10
                )
        ...
        if 10000 in results:
            assert not results[10000].get("accepted", False), \
    ...
    def test_rapid_circuit_breaker_triggers(self):
        ...
        for i in range(20):
            response = requests.post(
                ...
                json={
                    "text": f"Test {i}",
                    "openai_key": "invalid-key-to-trigger-failure"
                }
            )
    ...
    def test_response_consistency(self):
        ...
        for i in range(5):
            response = requests.post(
                f"{BASE_URL}/api/analyze",
                json={
                    "text": "Consistency test: What is 2+2?",
                    "openai_key": "test-key"
                }
            )
            if response.status_code == 200:
