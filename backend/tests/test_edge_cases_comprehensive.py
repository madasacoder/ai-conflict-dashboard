...
class TestBoundaryConditions:
    ...
    def test_zero_length_input_handling(self):
        ...
        for input_text in test_cases:
            ...
            assert tokens >= 0, f"Negative tokens for input: {repr(input_text)}"
            
            chunks = chunk_text(input_text, max_tokens=100)
            assert isinstance(chunks, list), f"chunk_text didn't return list for: {repr(input_text)}"
            
            # Empty input should produce empty or single empty chunk
            if not input_text.strip():
                assert len(chunks) <= 1, f"Too many chunks for empty input: {repr(input_text)}"
    ...
    def test_maximum_size_boundaries(self):
        ...
        for chunk in chunks:
            ...
            assert chunk_tokens <= max_chunk_size * 1.1, \
    ...
    def test_unicode_edge_cases(self):
        ...
        for char in problem_chars:
            ...
            assert tokens > 0, f"Failed to tokenize: {repr(char)}"
            
            chunks = chunk_text(text, max_tokens=10)
            assert all(isinstance(c, str) for c in chunks), \
                f"Invalid chunk type for: {repr(char)}"
            
            # Character should be preserved
            combined = "".join(chunks)
            assert char in combined or len(combined) > 0, \
                f"Character lost: {repr(char)}"
...
class TestUnexpectedInputCombinations:
    ...
    async def test_conflicting_parameters(self):
        ...
        assert response.status_code in [200, 400], \
            "Didn't handle conflicting models"
        ...
        assert response.status_code in [400, 422], \
            "Didn't validate invalid parameters"
    ...
    async def test_rapid_parameter_changes(self):
        ...
        param_sets = [
            ...
            {"text": "Much longer text " * 100, "key": "key2", "model": "gpt-4", "temperature": 0.9},
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                assert "api" in str(result).lower() or "key" in str(result).lower(), \
...
class TestSignalHandling:
    ...
    def test_handling_keyboard_interrupt(self):
        ...
        assert result == "interrupted gracefully", \
            "Didn't handle KeyboardInterrupt properly"
