...
describe('Real End-to-End Tests with Backend and Ollama', () => {
  ...
  describe('Real Node Creation and Workflow Building', () => {
    ...
    it('should execute real workflow with Ollama and display results', async () => {
      ...
      const { useWorkflowStore } = await import('../../state/workflowStore')
      const { addNode, createNewWorkflow } = useWorkflowStore.getState()
      
      // Create a workflow first (required for execution)
      createNewWorkflow('Test Translation Workflow', 'Testing workflow execution with Ollama')
      ...
      const executeButton = await waitFor(() => {
        const button = screen.getByTestId('execute-workflow')
        console.log('Execute button found:', button.textContent, 'disabled:', button.disabled)
        return button
      }, { timeout: 10000 })
      
      if ((executeButton as HTMLButtonElement).disabled) {
        console.log('Execute button is disabled - checking why...')
        const {workflow} = useWorkflowStore.getState()
        console.log('Current workflow:', workflow)
        throw new Error('Execute button is disabled - workflow not created properly')
      }
      ...
      await waitFor(() => {
        const status = screen.queryByTestId('execution-status')
        if (status) {
          console.log('Execution status found:', status.textContent)
        }
      }, { timeout: 10000 })
      ...
      await waitFor(() => {
        ...
        const complete = screen.queryByTestId('execution-complete')
        if (results) {
      ...
      }, { timeout: 60000 }) // Much longer timeout for real AI processing
